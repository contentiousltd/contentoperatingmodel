...
- **The current page marked with the underline, hover in colour** (ledger 33), as ltd: the sapling underline the text links share sits under the current page's link; hover and focus fade the text to sapling-650. The nav persists across pages and the script re-marks the current link once the view transition has finished (not at the swap, where the crossfade would hide the fade), so the underline fades from one link to the next over `--duration-glacial` in full view. A step in tone was tried first and read too quiet.
