...
- **The current page marked with the underline, hover in colour** (ledger 33), as ltd: the sapling underline the text links share sits under the current page's link; hover and focus fade the text to sapling-650. The nav persists across pages, sits outside the page crossfade (`transition:animate="none"`, since the crossfade's snapshot of the old page hid the old underline's fade) and the script re-marks the current link at the swap, so the underline fades from one link to the next from the first frame: in over `--motion-overlay`, out over `--duration-slow`. A step in tone was tried first and read too quiet.
